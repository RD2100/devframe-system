...
| R-011 | P1 | Paper private full text leaks into evidence or reports | Paper runtime/API privacy gate pinned at `145fc05`; unauthorized raw `paragraph_text`/`writelab_token` fails closed to `human_required`, and human-gate issue text is redacted | Add old evidence audit scans, full redacted reviewer-pack probes, and live WriteLab authorization tests before real paper content use. |
...
| R-014 | P2 | WriteLab handoff tests silently lose fixture coverage | Related test run shows 7 failures because `writelab_fixtures/mock_handoff.zip` is missing in the submodule path | Restore or generate the fixture and keep the handoff ZIP validation tests in CI. |
