...
|---|---|---|---|---|
| R-001 | P0 | `HUMAN_REQUIRED` or `BLOCKED` is reported as PASS | SADP negative fixtures and review rules; paper CLI final-acceptance labels, reviewer-pack non-authoritative boundary, and production finalizer `final_acceptance` boundary pinned at `ee08dd1` keep workflow/schema/artifact/report status separate from final acceptance | Extend the same boundary into agent-acceptance archive/final verdict integration. |
