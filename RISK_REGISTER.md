...
|---|---|---|---|---|
| R-001 | P0 | `HUMAN_REQUIRED` or `BLOCKED` is reported as PASS | SADP negative fixtures and review rules; paper CLI final-acceptance labels, reviewer-pack non-authoritative boundary, production finalizer `final_acceptance` boundary pinned through `4ab02c8`, and `agent-acceptance` SD-04 archive checks pinned at `1b1fad5` keep workflow/schema/artifact/report/test/zip status separate from final acceptance | Extend equivalent verdict-separation probes to future dispatch/test-frame runtime artifacts. |
...
| R-011 | P1 | Paper private full text leaks into evidence, reports, or live WriteLab payloads | Paper runtime/API privacy gate pinned at `145fc05`; audit ZIP candidate sensitive scan pinned at `cb34be3`; live WriteLab sensitive-input guard pinned at `ea0758a`; redacted reviewer-pack boundary pinned at `51215f1`; finalizer acceptance boundary pinned through `4ab02c8`; archive SD-04 boundary pinned at `1b1fad5`; unauthorized raw `paragraph_text`/`writelab_token` fails closed to `human_required`, human-gate issue text is redacted, audit packaging rejects unredacted state/ledger/report candidates, direct live WriteLab calls block before HTTP dispatch without explicit `paragraph_text` authorization, report output redacts WriteLab payload fragments and Bearer tokens, and graph/archive validation ignore reviewer-pack/artifact promotion claims | Keep real paper content blocked until fresh RuntimeAuthorization is issued. |
