...
| R-011 | P1 | Paper private full text leaks into evidence or reports | Paper runtime/API privacy gate pinned at `145fc05`; audit ZIP candidate sensitive scan pinned at `cb34be3`; unauthorized raw `paragraph_text`/`writelab_token` fails closed to `human_required`, human-gate issue text is redacted, and audit packaging rejects unredacted state/ledger/report candidates | Add full redacted reviewer-pack probes and live WriteLab authorization tests before real paper content use. |
