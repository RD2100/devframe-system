...
Date: 2026-06-15
Status: `SECURITY_PREFLIGHT_IN_PROGRESS`
...
|---|---|---|---|---|
| SEC-CLI-001 | P1 | Open | TaskSpec verification command strings can enter the execution chain without a strict allowlist/sandbox boundary. | Add command allowlist or structured verification runner before treating TaskSpec verification as executable. |
| SEC-CLI-002 | P1 | Open | `goal_review_recovered` can rely on OpenCode exit status as a safety verdict, which risks fake green. | Require structured reviewer verdict and independent gate evidence. |
| SEC-DAEMON-001 | P1 | Open | Queued tasks can trigger daemon write-type execution without a daemon-side permission/risk gate. | Add daemon authorization/risk gate before queued write execution. |
...
`A_NEXT_RECOMMENDATION: DEFER_UNTIL_SECURITY_PREFLIGHT_COMPLETE`

The next acceptance should switch to Paper Function Business Capability
Validation only after the P1 security blockers are fixed or explicitly
classified by human review.
...
## 6. Devframe-System Integration Branch Note
...
- superproject integration submodule: `D:\devframe-system\dev-frame-opencode`
  on `codex/paper-audit-privacy-hard-gate` at `8119c85`
...
|---|---|---|---|
| P1 security preflight findings | Command execution, fake-green verdict, and daemon write-gate risks remain open. | Main orchestrator plus human reviewer if live runtime is requested. | Fix the P1 findings in isolated slices before Paper Business Capability Validation. |
...
project in `SECURITY_PREFLIGHT_IN_PROGRESS`, fix or explicitly triage the P1
security findings, then switch the next available acceptance to Paper Function
