...
最快路径：先读 README 第一屏，再打开 `CURRENT_DELIVERY.md`，运行其中列出的验证命令，然后查看生成的证据报告。这样可以最快看到核心思路：GPT 负责方向，执行器产出结果，审查用证据说话。
...
| 正在审查一次交付 | 打开 `CURRENT_DELIVERY.md`，验证包，再看 `integration/reports/` |
