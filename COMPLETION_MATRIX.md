...
| 1B | `dev-frame-opencode` runtime authorization contract | Complete with boundary | Parent pins commit `ee08dd1`; RuntimeAuthorization/EvidenceManifest contracts include paper `data_policy`, and direct live WriteLab calls require explicit `paragraph_text` authorization before HTTP dispatch | Fresh authorization before live execution |
| 1B | Paper feature hardening | In progress with pinned gate, handoff fixture, audit scan, live WriteLab guard, CLI status boundary, redacted reviewer pack boundary, and finalizer acceptance boundary | Parent pins commit `ee08dd1`; 100 graph tests passed; 130 runtime/CLI tests passed; 173 CLI/report/audit regression tests passed; 278 paper runtime/graph/WriteLab regression tests passed; 115 audit/e2e regression tests passed | Remaining user-visible mojibake cleanup and archive-side governance integration |
