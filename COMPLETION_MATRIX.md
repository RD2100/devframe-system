...
| 1B | Paper feature hardening | Machine-readable business validation report artifact candidate | Parent pins opencode commit `08ac4f5`, agent-acceptance commit `f3abb20`, and test-frame commit `891b106`; main-thread checks passed for opencode business/report and CLI tests, test-frame 19 negative/contract tests, agent-acceptance 23 closure tests plus compileall, control-plane 14 dry-run tests, and parent read-only checks | Human review of candidate; SD-07 implementation; fresh RuntimeAuthorization before real paper content or live WriteLab execution |
...
| 5 | Human-gated live pilot | Not started | `agent-acceptance` returned `SD07_TASKSPEC_REQUIRED` for real-content/live WriteLab RuntimeAuthorization boundary | Requires SD-07 implementation and fresh RuntimeAuthorization |
