...
| 1B | Paper feature hardening | In progress with pinned gate, handoff fixture, audit scan, live WriteLab guard, CLI status boundary, redacted reviewer pack boundary, finalizer acceptance boundary, and focused mojibake cleanup | Parent pins commit `4ab02c8`; finalizer regression suites previously passed at `ee08dd1`; focused mojibake scan now leaves only legitimate BibTeX Latin mapping; goal runner parse check, goal runner tests, and diff check passed | Archive-side governance integration |
