...
| 1B | `dev-frame-opencode` runtime authorization contract | Complete with boundary | Parent pins commit `8119c85`; RuntimeAuthorization/EvidenceManifest contracts include paper `data_policy`, direct live WriteLab calls require explicit `paragraph_text` authorization before HTTP dispatch, post-run changed files are checked against effective write scope, and paper audit/report gates cover paragraph/matched-text/text-span payloads | Fresh authorization before live execution |
| 1B | Paper feature hardening | In progress with pinned gate, handoff fixture, audit scan, live WriteLab guard, CLI status boundary, redacted reviewer pack boundary, finalizer acceptance boundary, focused mojibake cleanup, archive-side verdict boundary, post-run write-set hard gate, paper audit privacy hard gate, and reviewer negative fixtures | Parent pins opencode commit `8119c85`, agent-acceptance commit `b505bf7`, and test-frame commit `be27de0`; main-thread checks passed for opencode 54 paper tests, agent-acceptance 22 closure tests, and test-frame 31 fixture/gate tests | Security Preflight P1 blockers; fresh authorization before live execution; Paper Business Capability Validation |
...
| 2-pre | Security Preflight | In progress with blockers | `integration/reports/security-preflight-2026-06-15.md`; no P0 found, P1 findings remain open | Fix or human-triage P1 findings before paper business acceptance |
