...
| 1B | `dev-frame-opencode` runtime authorization contract | Complete with boundary | Parent pins commit `ea0758a`; RuntimeAuthorization/EvidenceManifest contracts include paper `data_policy`, and direct live WriteLab calls require explicit `paragraph_text` authorization before HTTP dispatch | Fresh authorization before live execution |
| 1B | Paper feature hardening | In progress with pinned gate, handoff fixture, audit scan, and live WriteLab guard | Parent pins commit `ea0758a`; ruff passed for the changed client/tests; 276 paper/WriteLab regression tests passed; 115 paper privacy/audit regression tests passed | Full redacted reviewer pack shape and CLI UX matrix |
