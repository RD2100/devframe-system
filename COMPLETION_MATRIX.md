...
| 1B | `dev-frame-opencode` runtime authorization contract | Complete with boundary | Parent pins commit `3395033`; RuntimeAuthorization/EvidenceManifest contracts include paper `data_policy`, and direct live WriteLab calls require explicit `paragraph_text` authorization before HTTP dispatch | Fresh authorization before live execution |
| 1B | Paper feature hardening | In progress with pinned gate, handoff fixture, audit scan, live WriteLab guard, and CLI status boundary | Parent pins commit `3395033`; 151 CLI/report/audit regression tests passed; 276 paper runtime/graph/WriteLab regression tests passed | Full redacted reviewer pack shape |
