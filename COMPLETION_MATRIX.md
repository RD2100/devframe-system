...
| 1B | `dev-frame-opencode` runtime authorization contract | Complete with boundary | Parent pins commit `4ab02c8`; RuntimeAuthorization/EvidenceManifest contracts include paper `data_policy`, and direct live WriteLab calls require explicit `paragraph_text` authorization before HTTP dispatch | Fresh authorization before live execution |
| 1B | Paper feature hardening | In progress with pinned gate, handoff fixture, audit scan, live WriteLab guard, CLI status boundary, redacted reviewer pack boundary, finalizer acceptance boundary, focused mojibake cleanup, and archive-side verdict boundary | Parent pins opencode commit `4ab02c8` and agent-acceptance commit `1b1fad5`; finalizer regression suites previously passed at `ee08dd1`; focused mojibake scan now leaves only legitimate BibTeX Latin mapping; closure validator SD-04 rejects reviewer-pack/report/test/zip final acceptance promotion | Post-run `changed_files subset of write_set` hard gate; fresh authorization before live execution |
